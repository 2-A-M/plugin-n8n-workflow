...
async function resolveOneCredential(credType, userId, config, credStore, credProvider, apiClient, missingConnections, tagName) {
    ...
    if (cachedId) {
        logger.debug({ src: 'plugin:n8n-workflow:utils:credentials' }, `Resolved ${credType} from credential store`);
        return cachedId;
    }
    // 2. Static config
    ...
    if (credProvider) {
        try {
            ...
            if (result?.status === 'credential_data') {
                if (!apiClient) {
                    logger.error({ src: 'plugin:n8n-workflow:utils:credentials' }, `Received credential_data for ${credType} but no apiClient available`);
                ...
                try {
                    ...
                    logger.warn({ src: 'plugin:n8n-workflow:utils:credentials' }, `Failed to cache credential mapping for ${credType} (credential still usable): ${cacheError instanceof Error ? cacheError.message : String(cacheError)}`);
                }
                logger.info({ src: 'plugin:n8n-workflow:utils:credentials' }, `Created n8n credential for ${credType}: ${n8nCred.id}`);
                return n8nCred.id;
            }
            ...
            logger.error({ src: 'plugin:n8n-workflow:utils:credentials' }, `Credential provider failed for ${credType}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    // 4. Missing
    missingConnections.push({ credType });
 ...
 */
function findCredentialId(credentials, credType) {
    ...
    if (withoutApi !== credType && credentials[withoutApi]) {
        logger.debug({ src: 'plugin:n8n-workflow:utils:credentials' }, `Fuzzy credential match: "${credType}" → "${withoutApi}" (removed Api suffix)`);
    ...
    if (credentials[withApi]) {
        logger.debug({ src: 'plugin:n8n-workflow:utils:credentials' }, `Fuzzy credential match: "${credType}" → "${withApi}" (added Api suffix)`);
