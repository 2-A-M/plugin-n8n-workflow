...
import { loadOutputSchema, loadTriggerOutputSchema, parseExpressions, fieldExistsInSchema, getAllFieldPathsTyped, } from './outputSchema';
...
export function correctOptionParameters(workflow) {
    ...
    for (const node of workflow.nodes) {
        ...
        if (node.type !== nodeDef.name) {
            logger.warn({ src: 'plugin:n8n-workflow:correctOptions' }, `Node "${node.name}": type "${node.type}" → "${nodeDef.name}"`);
        ...
        if (node.typeVersion && !validVersions.includes(node.typeVersion)) {
            ...
            logger.warn({ src: 'plugin:n8n-workflow:correctOptions' }, `Node "${node.name}": typeVersion ${node.typeVersion} → ${maxVersion}`);
...
function fixOptionValue(node, prop) {
    ...
    const corrected = prop.default !== undefined && allowedValues.includes(prop.default)
        ? prop.default
        : allowedValues[0];
    logger.warn({ src: 'plugin:n8n-workflow:correctOptions' }, `Node "${node.name}": ${prop.name} "${currentValue}" → "${corrected}"`);
