...
export class N8nWorkflowService extends Service {
    ...
    static async start(runtime) {
        ...
        if (credentials) {
            ...
            if (configured.length > 0) {
                logger.info({ src: 'plugin:n8n-workflow:service:main' }, `Pre-configured credentials: ${configured.join(', ')}`);
            }
        }
        return service;
    }
    ...
    injectCatalogClarifications(workflow) {
        ...
        const nonCatalog = (workflow._meta.requiresClarification || []).filter((c) => !c.endsWith(CATALOG_SUFFIX));
        if (catalogWarnings.length > 0) {
            logger.warn({ src: 'plugin:n8n-workflow:service:main' }, `Catalog validation: ${catalogWarnings.join(', ')}`);
    ...
    async generateWorkflowDraft(prompt) {
        ...
        if (credProvider?.checkCredentialTypes) {
            ...
            if (credTypes.size > 0) {
                ...
                if (checkResult.unsupported.length > 0) {
                    ...
                    logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Feasibility OK: ${feasibility.reason}. Proceeding with ${remaining.length} nodes.`);
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            if (repairResult.errors.length === 0)
                break;
            if (attempt === 2) {
                logger.warn({ src: 'plugin:n8n-workflow:service:main', errors: repairResult.errors }, `validateAndRepair: ${repairResult.errors.length} unrecoverable error(s) after 3 retries — proceeding to deploy with _meta.errors`);
            ...
            try {
                ...
                logger.warn({ src: 'plugin:n8n-workflow:service:main', err: err instanceof Error ? err.message : String(err) }, 'fixWorkflowErrors threw — exiting retry loop');
                break;
            }
        }
        ...
        if (optionFixes > 0) {
            logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Corrected ${optionFixes} invalid option parameter(s)`);
        }
        const unknownParams = detectUnknownParameters(workflow);
        if (unknownParams.length > 0) {
            logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Found ${unknownParams.length} node(s) with unknown parameters, auto-correcting...`);
        ...
        if (invalidRefs.length > 0) {
            logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Found ${invalidRefs.length} invalid field reference(s), auto-correcting...`);
        ...
        if (exprPrefixed > 0) {
            logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Prefixed ${exprPrefixed} expression value(s) with "="`);
        }
    ...
    async modifyWorkflowDraft(existingWorkflow, modificationRequest) {
        logger.info({ src: 'plugin:n8n-workflow:service:main' }, `Modifying workflow draft: ${modificationRequest.slice(0, 100)}`);
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            if (repairResult.errors.length === 0)
                break;
            if (attempt === 2) {
                logger.warn({ src: 'plugin:n8n-workflow:service:main', errors: repairResult.errors }, `validateAndRepair (modify): ${repairResult.errors.length} unrecoverable error(s) after 3 retries`);
                workflow._meta = workflow._meta ?? {};
                const errorLines = repairResult.errors.map((e) => `${e.node}: ${e.detail}${e.availableFields?.length ? ` (available: ${e.availableFields.join(', ')})` : ''}`);
                const existing = workflow._meta.requiresClarification ?? [];
                workflow._meta.requiresClarification = [...existing, ...errorLines];
            ...
            try {
                ...
                logger.warn({ src: 'plugin:n8n-workflow:service:main', err: err instanceof Error ? err.message : String(err) }, 'fixWorkflowErrors (modify) threw — exiting retry loop');
                break;
            }
        }
        normalizeTriggerSimpleParam(workflow);
        const optionFixes = correctOptionParameters(workflow);
        if (optionFixes > 0) {
            logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Corrected ${optionFixes} invalid option parameter(s) in modified workflow`);
        }
        const unknownParams = detectUnknownParameters(workflow);
        if (unknownParams.length > 0) {
            logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Found ${unknownParams.length} node(s) with unknown parameters in modified workflow, auto-correcting...`);
            workflow = await correctParameterNames(this.runtime, workflow, unknownParams);
        }
        const invalidRefs = validateOutputReferences(workflow);
        if (invalidRefs.length > 0) {
            logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Found ${invalidRefs.length} invalid field reference(s) in modified workflow, auto-correcting...`);
            workflow = await correctFieldReferences(this.runtime, workflow, invalidRefs);
        }
        const exprPrefixed = ensureExpressionPrefix(workflow);
        if (exprPrefixed > 0) {
            logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Prefixed ${exprPrefixed} expression value(s) with "=" in modified workflow`);
        }
        const validationResult = validateWorkflow(workflow);
        if (!validationResult.valid) {
            logger.error({ src: 'plugin:n8n-workflow:service:main' }, `Modified workflow validation errors: ${validationResult.errors.join(', ')}`);
    ...
    async deployWorkflow(workflow, userId) {
        logger.info({ src: 'plugin:n8n-workflow:service:main' }, `Deploying workflow "${workflow.name}" for user ${userId}`);
        const config = this.getConfig();
        const client = this.getClient();
        const credStore = this.runtime.getService(N8N_CREDENTIAL_STORE_TYPE);
        const rawProvider = this.runtime.getService(N8N_CREDENTIAL_PROVIDER_TYPE);
        const credProvider = isCredentialProvider(rawProvider) ? rawProvider : null;
        ...
        if (workflow.id) {
            try {
                ...
                logger.warn({ src: 'plugin:n8n-workflow:service:main' }, `Update failed for workflow ${workflow.id}, creating new workflow instead`);
        ...
        logger.info({ src: 'plugin:n8n-workflow:service:main' }, `Workflow ${wasUpdate ? 'updated' : 'created'}: ${deployedWorkflow.id}`);
        // Activate (publish) the workflow immediately after creation/update
        let active = false;
        try {
            await client.activateWorkflow(deployedWorkflow.id);
            active = true;
            logger.info({ src: 'plugin:n8n-workflow:service:main' }, `Workflow ${deployedWorkflow.id} activated`);
        }
        catch (error) {
            logger.warn({ src: 'plugin:n8n-workflow:service:main' }, `Failed to activate workflow: ${error instanceof Error ? error.message : String(error)}`);
        }
        ...
        if (userId && !wasUpdate) {
            try {
                ...
                logger.debug({ src: 'plugin:n8n-workflow:service:main' }, `Tagged workflow ${deployedWorkflow.id} with "${tagName}"`);
            }
            catch (error) {
