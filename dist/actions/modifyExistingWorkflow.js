...
export const modifyExistingWorkflowAction = {
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        if (!service) {
            logger.error({ src: 'plugin:n8n-workflow:action:modify-existing' }, 'N8n Workflow service not available');
            if (callback) {
        ...
        try {
            ...
            const existingDraft = await runtime.getCache(cacheKey);
            if (existingDraft && Date.now() - existingDraft.createdAt < DRAFT_TTL_MS) {
                if (callback) {
                    await callback({
                        text: 'You already have a workflow draft in progress. ' +
            ...
            logger.info({ src: 'plugin:n8n-workflow:action:modify-existing' }, `Workflow match: ${matchResult.matchedWorkflowId || 'none'} (confidence: ${matchResult.confidence})`);
            ...
            logger.error({ src: 'plugin:n8n-workflow:action:modify-existing' }, `Failed to load workflow for modification: ${errorMessage}`);
...
//# sourceMappingURL=modifyExistingWorkflow.js.map