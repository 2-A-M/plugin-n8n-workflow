...
function buildFlowChain(connections) {
...
function buildPreviewData(workflow) {
    const creds = new Set();
...
function diffNodeParams(before, after) {
    const changes = {};
    for (const afterNode of after.nodes) {
        const beforeNode = before.nodes.find((n) => n.name === afterNode.name);
        const afterParams = (afterNode.parameters || {});
        const beforeParams = (beforeNode?.parameters || {});
        const nodeChanges = {};
...
export const createWorkflowAction = {
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        try {
            let existingDraft = await runtime.getCache(cacheKey);
            ...
            if (existingDraft) {
                ...
                if (existingDraft.originMessageId && existingDraft.originMessageId === message.id) {
                    logger.info({ src: 'plugin:n8n-workflow:action:create' }, 'Same message as draft origin — skipping');
                ...
                logger.info({ src: 'plugin:n8n-workflow:action:create' }, `Draft intent: ${intentResult.intent} — ${intentResult.reason}`);
                ...
                const effectiveIntent = intentResult.intent === 'confirm' &&
                    existingDraft.workflow._meta?.requiresClarification?.length
                    ? 'modify'
                    : intentResult.intent;
                if (effectiveIntent !== intentResult.intent) {
                    logger.info({ src: 'plugin:n8n-workflow:action:create' }, 'Draft has pending clarification — overriding "confirm" → "modify" to regenerate with user\'s answers');
                }
                switch (effectiveIntent) {
                    ...
                    case 'modify': {
                        ...
                        logger.info({ src: 'plugin:n8n-workflow:action:create' }, `Modifying draft: ${modification.slice(0, 100)}`);
                    ...
                    case 'new': {
                        ...
                        await runtime.deleteCache(cacheKey);
                        try {
                            return await generateAndPreview(runtime, service, generationContext, userId, cacheKey, message.id ?? '', callback);
                        }
                        catch (genError) {
                            logger.warn({ src: 'plugin:n8n-workflow:action:create' }, `New workflow generation failed — restoring previous draft: ${genError instanceof Error ? genError.message : String(genError)}`);
                    ...
                    default: {
                        logger.info({ src: 'plugin:n8n-workflow:action:create' }, 'Intent classification unclear — re-showing preview');
                        const text = await formatActionResponse(runtime, 'PREVIEW', buildPreviewData(existingDraft.workflow));
                        if (callback) {
                            await callback({ text, success: true });
                        }
                        return { success: true, data: { awaitingUserInput: true } };
...
async function generateAndPreview(runtime, service, prompt, userId, cacheKey, messageId, callback) {
    logger.info({ src: 'plugin:n8n-workflow:action:create' }, `Generating workflow from prompt: ${prompt.slice(0, 100)}...`);
