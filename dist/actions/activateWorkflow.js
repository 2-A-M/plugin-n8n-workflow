...
export const activateWorkflowAction = {
    ...
    handler: async (runtime, message, state, _options, callback) => {
        ...
        try {
            ...
            let pendingDraft = await runtime.getCache(cacheKey);
            ...
            if (pendingDraft) {
                ...
                if (pendingDraft.workflow._meta?.requiresClarification?.length) {
                    logger.info({ src: 'plugin:n8n-workflow:action:activate' }, 'Draft redirect: draft needs clarification, prompting user');
                    if (callback) {
                        await callback({
                ...
                logger.info({ src: 'plugin:n8n-workflow:action:activate' }, `Draft redirect: deploying pending draft "${pendingDraft.workflow.name}" (LLM misrouted to ACTIVATE)`);
                ...
                if (result.missingCredentials.length > 0) {
                    const connList = result.missingCredentials
                        .map((m) => m.authUrl ? `- **${m.credType}**: [Connect](${m.authUrl})` : `- **${m.credType}**`)
                        .join('\n');
                    if (callback) {
                        await callback({
