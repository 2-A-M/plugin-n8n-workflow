import { validateWorkflow, validateNodeParameters, validateNodeInputs, positionNodes, } from '../utils/workflow';
import { getService } from './_helpers';
/**
 * GET /workflows
 */
async function listWorkflows(req, res, runtime) {
    try {
 ...
 */
async function createWorkflow(req, res, runtime) {
    try {
        const { workflow, userId, activate } = req.body;
        if (!workflow || !userId) {
 ...
 * GET /workflows/:id
 */
async function getWorkflow(req, res, runtime) {
    try {
 ...
 */
async function updateWorkflow(req, res, runtime) {
    try {
        const id = req.params?.id;
        if (!id) {
            res.status(400).json({ success: false, error: 'workflow_id_required' });
 ...
 * DELETE /workflows/:id
 */
async function deleteWorkflow(req, res, runtime) {
    try {
        const id = req.params?.id;
        if (!id) {
            res.status(400).json({ success: false, error: 'workflow_id_required' });
 ...
 */
async function activateWorkflow(req, res, runtime) {
    try {
        const id = req.params?.id;
        if (!id) {
            res.status(400).json({ success: false, error: 'workflow_id_required' });
 ...
 */
async function deactivateWorkflow(req, res, runtime) {
    try {
        const id = req.params?.id;
        if (!id) {
            res.status(400).json({ success: false, error: 'workflow_id_required' });
