...
import { createWorkflowAction, getExecutionsAction, activateWorkflowAction, deactivateWorkflowAction, deleteWorkflowAction, modifyExistingWorkflowAction, } from './actions/index';
import { workflowStatusProvider, activeWorkflowsProvider, pendingDraftProvider, } from './providers/index';
...
export const n8nWorkflowPlugin = {
    ...
    init: async (_config, runtime) => {
        ...
        if (workflowSettings?.credentials) {
            const credCount = Object.keys(workflowSettings.credentials).filter((k) => workflowSettings.credentials[k]).length;
            logger.info({ src: 'plugin:n8n-workflow:plugin:init' }, `Pre-configured credentials: ${credCount} credential types`);
        }
