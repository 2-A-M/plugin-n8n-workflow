...
function discoverTriggers(): TriggerInfo[] {
  ...
  return nodes
    ...
    .map((n) => {
      ...
      const version = Array.isArray(n.version) ? Math.max(...n.version) : n.version;
...
async function main() {
  ...
  for (const trigger of triggersWithCreds) {
    ...
    const existingWf = existingWorkflows.find((w) => w.triggerType === trigger.nodeType);
    ...
    if (existingWf) {
      ...
      if (schema) {
        ...
        console.log(`   Captured ${fields.length} fields: ${fields.slice(0, 8).join(', ')}...`);
        console.log();
        continue;
      }
    ...
    if (createOnly) {
      if (!workflowId) {
        try {
          ...
          const created = await n8nRequest<{ id: string }>('POST', '/workflows', workflow);
          workflowId = created.id;
          console.log(`   Created: ${workflowId} (${N8N_HOST}/workflow/${workflowId})`);
        } catch (error) {
          console.log(`   ERROR creating: ${error instanceof Error ? error.message : error}`);
        }
      }
      result.stats.skipped++;
      console.log();
 ...
 */
async function findExistingCaptureWorkflows(): Promise<Array<{ id: string; triggerType: string }>> {
  try {
    ...
    const { data } = await n8nRequest<{ data: WorkflowListItem[] }>('GET', '/workflows?limit=50');
    return data
...
async function captureFromWorkflow(
  ...
  try {
    const { data: executions } = await n8nRequest<{ data: N8nExecution[] }>(
      'GET',
    ...
    const fullExec = await n8nRequest<N8nExecution>(
      'GET',
