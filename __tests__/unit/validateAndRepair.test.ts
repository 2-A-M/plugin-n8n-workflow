...
describe('validateAndRepair — authentication back-fill', () => {
  test('sets parameters.authentication when Gmail+gmailOAuth2 is attached but auth missing', () => {
    const def = makeNodeDef({
      ...
      version: [2, 2.1],
      credentials: [
        { name: 'gmailOAuth2', required: true, displayOptions: { show: { authentication: ['oAuth2'] } } },
        { name: 'googleApi', required: true, displayOptions: { show: { authentication: ['serviceAccount'] } } },
      ],
    });
    const wf = makeWorkflow({
      nodes: [{
  ...
  test('does not overwrite authentication when LLM already set it', () => {
    const def = makeNodeDef({
      ...
      credentials: [
        { name: 'discordBotApi', required: true, displayOptions: { show: { authentication: ['botToken'] } } },
      ],
    });
    const wf = makeWorkflow({
      nodes: [{
  ...
  test('skips back-fill when displayOptions has multiple auth values (ambiguous)', () => {
    const def = makeNodeDef({
      name: 'n8n-nodes-base.test',
      credentials: [
        { name: 'multiAuth', required: true, displayOptions: { show: { authentication: ['a', 'b'] } } },
      ],
    });
    const wf = makeWorkflow({
      nodes: [{
...
describe('validateAndRepair — node-name uniqueness', () => {
  test('renames duplicate node names with (2) suffix', () => {
    ...
    const wf = makeWorkflow({
      nodes: [
        { name: 'Step', type: 'n8n-nodes-base.test', typeVersion: 1, position: [0, 0], parameters: {} },
        { name: 'Step', type: 'n8n-nodes-base.test', typeVersion: 1, position: [200, 0], parameters: {} },
      ],
    });
    const r = validateAndRepair(wf, [def], NO_CTX);
...
describe('validateAndRepair — connection sanity', () => {
  test('drops edges to non-existent nodes', () => {
    ...
    const wf = makeWorkflow({
      nodes: [
        { name: 'A', type: 'n8n-nodes-base.test', typeVersion: 1, position: [0, 0], parameters: {} },
        { name: 'B', type: 'n8n-nodes-base.test', typeVersion: 1, position: [200, 0], parameters: {} },
      ],
      connections: {
        A: {
          main: [[
