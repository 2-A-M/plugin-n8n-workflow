...
describe('loadOutputSchema', () => {
  test('loads Gmail message/getAll schema', () => {
    const result = loadOutputSchema('n8n-nodes-base.gmail', 'message', 'getAll');
  ...
  test('returns null for unknown node', () => {
    const result = loadOutputSchema('n8n-nodes-base.unknownNode', 'resource', 'op');
    expect(result).toBeNull();
  });

  test('returns null for unknown resource', () => {
    const result = loadOutputSchema('n8n-nodes-base.gmail', 'unknownResource', 'op');
    expect(result).toBeNull();
  });

  test('returns null for unknown operation', () => {
    const result = loadOutputSchema('n8n-nodes-base.gmail', 'message', 'unknownOp');
    expect(result).toBeNull();
  });
});
...
describe('parseExpressions', () => {
  ...
  test('extracts multiple expressions from same string', () => {
    const params = { message: 'From: {{ $json.from.text }} Subject: {{ $json.subject }}' };
    const refs = parseExpressions(params);
...
describe('fieldExistsInSchema', () => {
  ...
  test('finds nested object field', () => {
    expect(fieldExistsInSchema(['from', 'text'], mockGmailMessageSchema)).toBe(true);
  });

  test('finds array item field', () => {
    expect(fieldExistsInSchema(['from', 'value', '0', 'address'], mockGmailMessageSchema)).toBe(
      true
    );
  });
  ...
  test('returns false for non-existent nested field', () => {
    expect(fieldExistsInSchema(['from', 'email'], mockGmailMessageSchema)).toBe(false);
  });
...
describe('loadTriggerOutputSchema', () => {
  ...
  test('returns null for empty schema (Google Sheets)', () => {
    const result = loadTriggerOutputSchema('n8n-nodes-base.googleSheetsTrigger');
    // Sheets had an empty execution — no properties
    expect(result).toBeNull();
  });

  test('returns null when simple=false (raw mode differs from captured schema)', () => {
    const result = loadTriggerOutputSchema('n8n-nodes-base.gmailTrigger', { simple: false });
    expect(result).toBeNull();
  });

  test('loads schema when simple=true (matches captured schema)', () => {
    const result = loadTriggerOutputSchema('n8n-nodes-base.gmailTrigger', { simple: true });
    expect(result).not.toBeNull();
    expect(result!.fields).toContain('Subject');
  });
...
describe('langchain OpenAI schemas', () => {
  test('loads text/response schema with correct output structure', () => {
    const result = loadOutputSchema('@n8n/n8n-nodes-langchain.openAi', 'text', 'response');
    expect(result).not.toBeNull();
    expect(result!.fields).toContain('output');
    // Verify nested structure: output[0].content[0].text exists
    expect(fieldExistsInSchema(['output', '0', 'content', '0', 'text'], result!.schema)).toBe(true);
  });
  ...
  test('detects wrong field path choices[0].message.content', () => {
    ...
    expect(fieldExistsInSchema(['choices', '0', 'message', 'content'], result!.schema)).toBe(false);
  });

  test('loads text/classify schema', () => {
    const result = loadOutputSchema('@n8n/n8n-nodes-langchain.openAi', 'text', 'classify');
    expect(result).not.toBeNull();
    expect(result!.fields).toContain('flagged');
  });

  test('loads image/generate schema', () => {
    const result = loadOutputSchema('@n8n/n8n-nodes-langchain.openAi', 'image', 'generate');
    expect(result).not.toBeNull();
  ...
  test('loads audio/transcribe schema', () => {
    const result = loadOutputSchema('@n8n/n8n-nodes-langchain.openAi', 'audio', 'transcribe');
    expect(result).not.toBeNull();
  ...
  test('lists available operations for text resource', () => {
    const ops = getAvailableOperations('@n8n/n8n-nodes-langchain.openAi', 'text');
